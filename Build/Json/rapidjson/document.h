...
public:
    ...
    */
    ...
    template <unsigned parseFlags, typename SourceEncoding, typename InputStream>
    GenericDocument& ParseStreamFirst(InputStream& is) {
        ValueType::SetNull(); // Remove existing root if exist
        GenericReader<SourceEncoding, Encoding, StackAllocator> reader(&stack_.GetAllocator());
        ClearStackOnExit scope(*this);
        parseResult_ = reader.template ParseFirst<parseFlags>(is, *this);
        if (parseResult_) {
            RAPIDJSON_ASSERT(stack_.GetSize() == sizeof(ValueType)); // Got one and only one root object
            this->RawAssign(*stack_.template Pop<ValueType>(1));    // Add this-> to prevent issue 13.
        }
    ...
    template <unsigned parseFlags, typename SourceEncoding>
    GenericDocument& ParseFirst(const Ch* str) {
        RAPIDJSON_ASSERT(!(parseFlags & kParseInsituFlag));
        GenericStringStream<SourceEncoding> s(str);
        return ParseStreamFirst<parseFlags, SourceEncoding>(s);
    }

    template <unsigned parseFlags>
    GenericDocument& ParseFirst(const Ch* str) {
        return ParseFirst<parseFlags, Encoding>(str);
    }
    ...
    */
    GenericDocument& ParseFirst(const Ch* str) {
        return ParseFirst<kParseDefaultFlags>(str);
    }
    //add end----------
